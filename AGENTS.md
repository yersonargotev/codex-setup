...
<!-- gentle-ai:codegraph-mode -->

CodeGraph Mode: enabled

If `.codegraph/` exists in the project, answer architecture, trace, impact, and symbol-discovery questions with CodeGraph first instead of re-deriving the same answer with grep/read loops.
...
Treat CodeGraph-returned source as already read. Use raw Read/Grep only to verify details CodeGraph did not cover. If `.codegraph/` does not exist, ask before running `codegraph init -i`.
